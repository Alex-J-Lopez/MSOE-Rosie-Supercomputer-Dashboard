...
import { MemoryStick, Database, RefreshCw, AlertCircle } from 'lucide-react';
import { useMemoryUsage, useMemoryCapacity, useNodes } from '../hooks/useMemoryUsage';

interface MemoryUsageComponentProps {
  className?: string;
}

// Custom gauge component for memory usage
const MemoryGauge: React.FC<{ value: number }> = ({ value }) => {
...
const MemoryUsageComponent: React.FC<MemoryUsageComponentProps> = ({ className = '' }) => {
  ...
  const { data: usageData, isLoading: usageLoading, error: usageError } = useMemoryUsage(selectedNode);
  const { data: capacityData, isLoading: capacityLoading, error: capacityError } = useMemoryCapacity(selectedNode);

  const isLoading = usageLoading || capacityLoading;
  const error = usageError || capacityError;

  // Calculate used and free memory for pie chart
  const pieChartData = React.useMemo(() => {
    ...
    const usedBytes = (capacityData * usedPercentage) / 100;
    const freeBytes = capacityData - usedBytes;
    ...
    return [
      {
        ...
        color: usedPercentage < 50 ? '#10b981' : usedPercentage < 80 ? '#f59e0b' : '#ef4444'
      },
      {
        name: 'Free Memory', 
        value: freeBytes,
  ...
  if (error) {
    return (
      <div className={`bg-white rounded-lg shadow-lg p-6 ${className}`}>
        <div className="flex items-center space-x-2 mb-4">
          <MemoryStick className="h-6 w-6 text-red-600" />
          <h3 className="text-lg font-semibold text-gray-900">Memory Usage</h3>
    ...
    <div className={`bg-white rounded-lg shadow-lg p-6 ${className}`}>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <select
          ...
          className="px-3 py-1 border border-gray-300 rounded-md text-sm text-black focus:outline-none focus:ring-2 focus:ring-purple-500"
          aria-label="Select node"
          title="Select a node to view memory usage"
        >
          {nodes.map((node) => (
      ...
      {!usageData || !capacityData || isLoading || usageData.value === null || usageData.value === undefined ? (
        <div className="flex items-center justify-center h-64">
          <div className="text-gray-500">
            {isLoading ? 'Loading memory data...' : 'No data available'}
          </div>
        </div>
      ) : (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="flex items-center justify-center">
              <div className="flex items-center space-x-4">
                <MemoryGauge value={usageData.value} />
                <div>
